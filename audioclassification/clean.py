    ...
    for _cls in classes:
        ...
        for fn in tqdm(os.listdir(src_dir)):
            ...
            # discard the ending audio if it is too short
                ...
                for n,skip in enumerate(np.arange(0,delta_sample, delta_sample/10)):
                    trunc = (wav.shape[0] - skip) % delta_sample
                    for cnt, i in enumerate(np.arange(skip, wav.shape[0]-trunc, delta_sample)):
                        start = int(i)
                        stop = int(i + delta_sample)
                        sample = wav[start:stop]
                        save_sample(sample, rate, target_dir, fn, cnt*(n+1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dst_root', type=str, default='clean2',
                        help='directory to put audio files split by delta_time')
    parser.add_argument('--delta_time', '-dt', type=float, default=0.3,
                        help='time in seconds to sample audio')
    parser.add_argument('--sr', type=int, default=16000,
                        help='rate to downsample audio')

    parser.add_argument('--fn', type=str, default='sphere-001',
